...
assert(empty.includeDiscontinued === false, "パラメータなしなら廃止済み観測所は含めない（フェーズ16）");
const withDiscontinued = parseStateFromUrl(new URLSearchParams("discontinued=1"));
assert(withDiscontinued.includeDiscontinued === true, "discontinued=1で廃止済み観測所を含める状態を復元する");
const invalidDiscontinued = parseStateFromUrl(new URLSearchParams("discontinued=yes"));
assert(invalidDiscontinued.includeDiscontinued === false, "discontinued=1以外の値は含めない扱いにする");

// --- buildQueryString -----------------------------------------------------

assert(
  buildQueryString({ selectedPrefectures: new Set(), selectedElements: new Set(), keyword: "", page: 1 }) === "",
...
assert(!qsDefaultMode.includes("discontinued="), "含めない（既定値）のときはdiscontinuedパラメータを省略する");
const qsWithDiscontinued = buildQueryString({
  selectedPrefectures: new Set(),
...
assert(qsWithDiscontinued.includes("discontinued=1"), "含める場合はdiscontinued=1パラメータを含む");
