...
マッチング方法:
  ...
  6. scripts/extra_stations.json で precNo/blockNo を手書きしている観測所（南極・昭和基地など。
     block_numbers.json は国内47都道府県のページから収集しているため候補が存在しない）は
     照合対象から除外し、手書きの値をそのまま残す
...
def load_manual_ids(path):
    """precNo/blockNo を手書きで持っている観測所のID集合（照合をスキップする対象）"""
    try:
        raw = json.load(open(path, encoding="utf-8"))
    except FileNotFoundError:
        return set()
    return {s["id"] for s in raw.get("stations", []) if s.get("precNo") and s.get("blockNo")}
...
def main():
    ...
    parser.add_argument("--extra", default="scripts/extra_stations.json")
    ...
    manual_ids = load_manual_ids(args.extra)
    ...
    print(f"skipped (手書きのprecNo/blockNoを保持): {skipped_manual} / {total}")
